...
const path = 'D:/3. Project/App/Potato English Center/Dashboard/script.js';
let content = fs.readFileSync(path, 'utf8');

const filterReplace = `let rowsToRender = window.currentMonthFilteredRows || [];
...
content = content.replace(/let rowsToRender = window\.currentMonthFilteredRows \|\| \[\];[\s\S]*?renderTeacherPerformance\(rowsToRender, monthStr\);/, filterReplace);

const visReplace = `if (!isAdmin && !isLeader && currentUser) {
...
content = content.replace(/if \(!isAdmin && !isLeader && currentUser\) \{[\s\S]*?selectedTeacher = tSelect\.value;\n        \}\n    \}/, visReplace);

const oldRenderFuncStart = content.indexOf('function renderTeacherPerformance(classRows, currentMonthStr) {');
const nextFuncStart = content.indexOf('function renderAcademicPerformance(classRows) {');

if (oldRenderFuncStart !== -1 && nextFuncStart !== -1) {
    const before = content.substring(0, oldRenderFuncStart);
    const after = content.substring(nextFuncStart);
    
    const newRenderFunc = `function renderTeacherPerformance(classRows, currentMonthStr) {
